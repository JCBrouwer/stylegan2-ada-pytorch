...
def speedtest(seeds: Optional[List[int]], truncation_psi: float, noise_mode: str, outdir: str, z_dim: int, c_dim: int, w_dim: int, img_resolution: int, img_channels: int, synthesis_kwargs: dict):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print()
    print(device, torch.cuda.is_available())
    network_pkl = "https://nvlabs-fi-cdn.nvidia.com/stylegan2-ada-pytorch/pretrained/metfaces.pkl"

    G = Generator(z_dim = z_dim, c_dim = c_dim, w_dim = w_dim, img_resolution = img_resolution, img_channels = img_channels, synthesis_kwargs=synthesis_kwargs)
    G = G.to(device)
...
if __name__ == "__main__":
    ...
    for i in range(n):
        ...
        #Start timer
        timestart = time.time()
        summary = speedtest(seeds = seeds, truncation_psi=trunc, noise_mode=noise_mode, outdir=outdir, z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, synthesis_kwargs=synthesis_kwargs)  # pylint: disable=no-value-for-parameter
        #End timer
        timeend = time.time()
        torch.cuda.empty_cache()
